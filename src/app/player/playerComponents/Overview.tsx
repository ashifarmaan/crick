...
export default function Overview({playerAdvanceStats, playerStats, urlString, ranking}: Overview) {
  ...
  const playerDebuData = playerAdvanceStats?.player?.debut_data;

  const playerRole = profile?.playing_role == 'bat' ? "Batting" : profile?.playing_role == 'bowl' ? "Bowler" : profile?.playing_role == "all" ? "All- Rounder" :"";
  ...
  if(profile?.playing_role == 'bat'){
    odiRank = playerRank?.batsmen?.odis?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    testRank = playerRank?.batsmen?.tests?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    t20Rank = playerRank?.batsmen?.t20s?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
  }else if (profile?.playing_role == 'bowl'){
    odiRank = playerRank?.bowlers?.odis?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    testRank = playerRank?.bowlers?.tests?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    t20Rank = playerRank?.bowlers?.t20s?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
  }else if (profile?.playing_role == 'all'){
    odiRank = playerRank?.['all-rounders']?.odis?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    testRank = playerRank?.['all-rounders']?.tests?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
    t20Rank = playerRank?.['all-rounders']?.t20s?.find((item: { pid: number }) => Number(item.pid) === profile?.pid)?.rating;
  }
  
  
    const battingArray = Object.keys(playerBatting).map((format) => ({
      format: format.toUpperCase(),
      ...playerBatting[format]
    }));

    const bowlingArray = Object.keys(playerBowling).map((format) => ({
      format: format.toUpperCase(),
      ...playerBowling[format]
    }));

  console.log('Test', battingArray);
  ...
  const slides = last10Match?.batting.map((match: any, index: number) => ({
    id: index,
    text: `${match.runs}(${match.balls})`, // Assuming match.runs and match.balls are properties
    match: match.match_title
  }));

  const slidesBol = last10Match?.bowling.map((match: any, index: number) => ({
    id: index,
    text: `${match.runs}-${match.wickets}(${match.overs})`, // Assuming match.runs and match.balls are properties
    match: match.match_title
  }));
          ...
          <div id="overview">
            <div className="md:grid grid-cols-12 gap-4">
              ...
              <div className="lg:col-span-8 md:col-span-7">
                ...
                {playerDebuData.length > 0 &&
                <div className=" rounded-lg p-4 bg-[#ffffff] mb-4">
                  <div className="relative overflow-x-auto">
                    <table className="w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
                      <tbody>
                        {playerDebuData.map((debut:any, index:number) => (
                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={index}>
                          <th
                            scope="row"
                            className="px-6 py-2 text-[#586577] whitespace-nowrap dark:text-white font-normal"
