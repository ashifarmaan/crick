...
interface Live {
  ...
  export default function Live({
    ...
    let thisOverRun = commentaries.filter((events: {event: string;  over: any; }) => Number(events.over) === currentOver && events.event !== 'overend');
    let lastOverRun = commentaries.filter((events: {event: string; over: any; }) => Number(events.over) === lastOver && events.event !== 'overend');
    let thisOvertotalRuns = thisOverRun.reduce((accumulator: number, currentEvent: { run: number; }) => {
      return accumulator + currentEvent.run;
    }, 0);
    let lastOvertotalRuns = lastOverRun.reduce((accumulator: number, currentEvent: { run: number; }) => {
      return accumulator + currentEvent.run;
    }, 0);
    // const players = matchStates?.players;

    if(matchLiveData !== undefined && matchLiveData?.match_id == match_id && matchLiveData?.live?.live_inning !== undefined && matchLiveData?.live?.live_inning !== ''){
      ...
      thisOverRun = commentaries.filter((events: {event: string;  over: any; }) => Number(events.over) === currentOver && events.event !== 'overend');
      lastOverRun = commentaries.filter((events: {event: string; over: any; }) => Number(events.over) === lastOver && events.event !== 'overend');
      thisOvertotalRuns = thisOverRun.reduce((accumulator: number, currentEvent: { run: number; }) => {
        return accumulator + currentEvent.run;
      }, 0);
      lastOvertotalRuns = lastOverRun.reduce((accumulator: number, currentEvent: { run: number; }) => {
        return accumulator + currentEvent.run;
      }, 0);
    ...
    for (let i = 0; i < numberOfSpans; i++) {
      emptySpans.push(
        <span className="px-2 py-1 border rounded text-gray-700" key={`empty-${i}`}>
          {/* Empty span */}&nbsp;&nbsp;&nbsp;
        </span>
      );
    }
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-medium">
                          ...
                          {currPartnership?.batsmen?.[0]?.runs}{" "}
                            ...
                            {matchinfo?.batsmen?.[0]?.batsman_id == currPartnership?.batsmen?.[0]?.batsman_id ? (
                            <Image
                      ...
                      <div className="flex items-center justify-end flex-row-reverse gap-3">
                        ...
                        <div className="font-medium text-end">
                          ...
                          {currPartnership?.batsmen?.[1]?.runs}{" "}
                            ...
                            {matchinfo?.batsmen?.[0]?.batsman_id == currPartnership?.batsmen?.[1]?.batsman_id ? (
                            <Image
                              src="/assets/img/home/bat.png"
                              className="h-[14px]"
                              width={14} height={14} alt=""
                ...
                <Link href="/profile">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="font-medium">
                      ...
                      <p className="md:text-[15px] text-[14px] flex items-center">
                      {matchinfo?.bowlers?.[0]?.wickets}-{matchinfo?.bowlers?.[0]?.runs_conceded}{" "}
              ...
              <div className="rounded-lg bg-white p-4 flex lg:flex-row flex-col items-center md:gap-8 gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className={`px-2 py-1 border rounded ${lastOver.score == 6 ? ("bg-[#13b76dbd] text-white"): lastOver.score == 4 ?( "bg-orange-500 text-white"): lastOver.score == 'w' ?( "bg-red-500 text-white"):( "text-gray-700")}`} key={index}>
                      {lastOver.score}
                    </span>
                    ))}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className={`px-2 py-1 border rounded ${thisOver.score == 6 ? ("bg-[#13b76dbd] text-white"): thisOver.score == 4 ?( "bg-orange-500 text-white"): thisOver.score == 'w' ?( "bg-red-500 text-white"):( "text-gray-700")}`} key={index}>
                      {thisOver.score}
                    </span>
                    ))}
                  ...
                  <div>
                    <div className="flex justify-between items-center">
                      ...
                      <p className="text-[#909090]">
                        Overs left today:{" "}
                        <span className="font-semibol text-black">{remainingOvers}</span>
                      </p>
                    </div>
                    <div className="relative mt-4 h-[4px] bg-gray-200 overflow-hidden">
                      ...
                      <div
                        ...
                        style={{ width: `${teamwinpercentage?.draw}%`, left: `${teamwinpercentage?.team_a_win}%` }}
                      ></div>
                      <div
                        className="absolute h-full bg-[#B7132B]"
                        style={{ width: `${teamwinpercentage?.team_b_win}%`, left: `${(teamwinpercentage?.draw)+(teamwinpercentage?.team_a_win)}%` }}
                      ></div>
                    </div>
                    <div className="flex justify-between mt-2 text-sm">
                      <p className="text-green-600 font-medium">{matchDetails?.teama?.short_name}: {teamwinpercentage?.team_a_win}%</p>
                      <p className="text-yellow-600 font-medium">Drew: {teamwinpercentage?.draw}%</p>
                      <p className="text-red-600 font-medium">{matchDetails?.teamb?.short_name}: {teamwinpercentage?.team_b_win}%</p>
                    </div>
                  </div>
                </div>
        ...
        <div className="cust-box-click-container">
          ...
          <div className="cust-box-click-content">
            <div key="commetray">
              ...
              {commentaries.map((comments: any , index:number) => (
                comments?.event === "overend" ? (                  
              <div className="rounded-lg bg-white p-4 mb-4" key={index}>
                <div className="flex md:flex-row flex-col justify-between md:items-center mb-4 gap-2" >
                  ...
                  <div className="text-[14px] font-normal">
                    R Sharma: {comments?.bats?.[0]?.runs} <span className="text-[#909090]">({comments?.bats?.[0]?.balls_faced})</span> | S
                    Gill: {comments?.bats?.[1]?.runs} <span className="text-[#909090]">({comments?.bats?.[1]?.balls_faced}) </span>
                  </div>
                  <div className="text-[14px] font-normal">
                    Taskin Ahmed{" "}
                    <span className="text-[#909090]">{comments?.bowls?.[0]?.overs}-{comments?.bowls?.[0]?.runs_conceded}-{comments?.bowls?.[0]?.wickets}</span>
                  </div>
                </div>
                
                
                {commentaries.map((comment: any , index:number) => (
                  comment?.event !== "overend" && JSON.parse(comment?.over) === (comments.over -1) ? (

                    <><div className="border-t-[1px] border-[#E7F2F4]" /><div className="md:flex items-start py-3 md:px-3 gap-[21px]" key={comment.event_id}>
                    <div className="flex items-center gap-[10px] md:py-4 pb-4">
                      <p className="text-[16px] font-semibold">{comment?.over}.{comment?.ball}</p>
                      <p className={`text-[16px] font-semibold px-[11px] py-[2px rounded-lg ${comment?.run == 6 ? ("bg-[#13b76dbd] text-white"): comment?.run == 4 ?( "bg-orange-500 text-white"): comment?.run == 'w' ?( "bg-red-500 text-white"):( " text-white bg-[#bec2d3]")}`}>
                        {comment?.run}{" "}
                      </p>
                    </div>
                    <div>
