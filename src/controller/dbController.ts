...
export async function InsertOrUpdateMatchesSqauds_NotUSE() {
  try {
      ...
      limit(async () => {
        try {
          ...
          return {
            ...
            teamaSquad: JSON.stringify(
              data?.response?.["match-playing11"]?.teama?.squads || {}
            ).replace(/'/g, "\\'"),
            teambSquad: JSON.stringify(
              data?.response?.["match-playing11"]?.teamb?.squads || {}
            ).replace(/'/g, "\\'"),
          };
        } catch (error) {
    ...
    const values = filteredMatches
      .map(
        (m) =>
          `(${m.match_id}, '${m.teamaId}', '${m.teambId}', '${m.teamaSquad}', '${m.teambSquad}')`
      )
      .join(", ");
    const query = `
...
export async function UpdateManOfTheMatches() {
  try {
    ...
    console.log(
      `Updating ${filteredMatches.length} matches in the database...`
    );
    ...
    SET 
        man_of_the_match_pid = CASE 
            ${filteredMatches
              .map((m) => `WHEN match_id = ${m.match_id} THEN '${m.pid}'`)
              .join(" ")}
        END,
        man_of_the_match_pname = CASE 
            ${filteredMatches
              .map(
                (m) =>
                  `WHEN match_id = ${m.match_id} THEN '${m.name.replace(
                    /'/g,
                    "''"
                  )}'`
              )
...
export async function MatchInfo() {
  try {
    // const matchQuery = `SELECT match_id FROM matches WHERE match_id = 87999`;
...
export async function readMatchData() {
  ...
  const [rows]: [any[], any] = await db.execute(
    "SELECT * FROM match_info WHERE match_id = ? LIMIT 1",
    [matchId]
  );
  const filePath = rows[0].fileName;
  // console.log("test", rows[0].fileName);
  try {
    // const filePath = `D:\\MatchData\\match_${matchId}.json`;
